  ...
  for (const loan of activeLoans) {
    ...
    try {
      ...
      while (currentPaidMonths < loan.termMonths) {
        ...
        // Step 4: 기록탭에 지출 자동 기록 (linkedAssetId 유무 불문, 항상 실행)
...
/**
 * 대출 상환 과거 지출 기록 소급 생성 (backfill)
 *
 * - 이미 paidMonths > 0인 대출에 대해, 과거 회차 지출 기록이 없으면 생성
 * - 자산 차감 없음 (이미 처리된 과거 기록)
 * - paidMonths 변경 없음
 * - linkedLoanId + date로 중복 검사
 */
export async function backfillLoanExpenseRecords(): Promise<{
  created: number;
  skipped: number;
  errors: string[];
}> {
  ...
  const { loans } = useDebtStore.getState();
  const { records } = useLedgerStore.getState();

  // paidMonths > 0 이고 취소되지 않은 대출 필터
  const loansToBackfill = loans.filter(
    (loan) => loan.status !== 'cancelled' && (loan.paidMonths ?? 0) > 0
  );
  ...
  for (const loan of loansToBackfill) {
    try {
      ...
      // 1~paidMonths 회차에 대해 지출 기록 존재 여부 확인
      for (let month = 1; month <= loan.paidMonths; month++) {
        const payment = schedule.find((s) => s.month === month);
        if (!payment) continue;

        // 중복 검사 1: linkedLoanId + date
        const existingByLink = records.find(
          (r) =>
            r.type === 'expense' &&
        ...
        // 중복 검사 2: memo 패턴 매칭 (linkedLoanId 없는 구버전 기록 대비)
        const memoPattern = loan.name;
        const monthPattern = `(${month}/${loan.termMonths}`;
        const existingByMemo = records.find(
          (r) =>
            r.type === 'expense' &&
            r.date === payment.date &&
            r.memo?.includes(memoPattern) &&
            r.memo?.includes(monthPattern)
        );
        if (existingByMemo) {
          result.skipped++;
          continue;
        ...
        await addExpense({
          ...
          installmentId: null,
          memo,
          linkedAssetId: null, // 과거 기록이므로 자산 차감 안 함
        ...
        result.created++;
        console.log(
          `[Backfill] 대출 ${loan.name}: ${month}/${loan.termMonths}회차 지출 기록 소급 생성`
        );
      }
    } catch (error) {
      const errorMsg = `대출 ${loan.name} backfill 실패: ${error}`;
      result.errors.push(errorMsg);
  ...
  if (result.created > 0) {
    console.log(`[Backfill] 완료: ${result.created}건 생성, ${result.skipped}건 스킵`);
  }
